...
def runtime_env():
    ...
    # The bundled OpenSSL is deliberately built with an ssl dir that does not exist
    # on the CI runners, so without this, any HTTPS connection made from calibre code
    # fails with CERTIFICATE_VERIFY_FAILED. The certs are generated by the cacerts
    # sub-command of bootstrap, which is run before this.
    cadir = os.path.abspath('resources/mozilla-ca-certs')
    if not os.path.isdir(cadir):
        raise SystemExit(f'The Mozilla CA certs are missing from: {cadir}')
